...
This manuscript reports what has actually been built and run: a fully specified and unit-validated stability-metric framework, a wired-up ID-mapping matrix and enrichment-battery interface, and a synthetic illustration of the mechanics. It deliberately does not report pathway-level findings on GSE52778 or any other real dataset, because the Bioconductor packages the empirical pipeline depends on (\texttt{DESeq2}, \texttt{org.Hs.eg.db}, \texttt{clusterProfiler}, \texttt{ReactomePA}, \texttt{biomaRt}, \texttt{fgsea}) were not installable in the network-restricted environment used to prepare this manuscript. The \texttt{targets} pipeline that would produce those findings is complete and reviewable in the linked repository, and is exercised as a continuous-integration smoke test in an environment where those packages can be installed; running it end to end against GSE52778 and the two additional planned datasets (GSE159094, a PBMC dexamethasone series, and GSE80651, a podocyte series) to answer whether pathway fragility is a property of the pathway's gene-set definition or of a single dataset's noise, and anchoring the glucocorticoid case study against ChIP-seq-derived GR/NR3C1 target gene sets, is the immediate next step and is not claimed here.
...
No new biological data were generated. The \texttt{airway} dataset (GSE52778) analyzed by the accompanying, not-yet-executed pipeline is publicly available via the Bioconductor \texttt{airway} data package and the NCBI Gene Expression Omnibus (accession GSE52778). All code, the methods note, and the synthetic illustrative-example data underlying Table~\ref{tab:toy} and Fig.~\ref{fig:heatmap} are available at \url{https://github.com/ethanyyy123/bioadvances}.

\section{Author contributions statement}
E.Y.Y.\ designed the audit framework, implemented the software and tests, and wrote the manuscript.
