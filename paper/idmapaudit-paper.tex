...
\item an open-source R implementation whose stability-metric core has no Bioconductor dependency and is unit-tested against hand-computed values, including regression tests for four bugs found during development (Section~\ref{sec:validation}); and
...
\end{tabular*}
\begin{tablenotes}
\item[$^{*}$] One expectation in \texttt{enrichment-battery} requires \texttt{clusterProfiler} and is conditionally skipped (not counted as passed or failed) in a Bioconductor-free environment; the count shown is from the continuous-integration run where the full stack is installed.
\end{tablenotes}
\end{table}
...
Pathway-level fragility scores (GO/KEGG/Reactome/WikiPathways) on real data are not yet reported end to end in this manuscript. The remaining obstacle is not annotation-data availability: the \texttt{targets} pipeline's continuous-integration job installs the full dependency stack (\texttt{DESeq2}, \texttt{org.Hs.eg.db}, \texttt{clusterProfiler}, \texttt{ReactomePA}, \texttt{biomaRt}, \texttt{fgsea}, \texttt{airway}) without error on a standard GitHub Actions runner, and the differential-expression and ID-mapping-matrix stages run to completion against real data. Three implementation-level issues, each found from an actual pipeline run rather than by inspection and each now fixed and covered by a regression test (Section~\ref{sec:validation}), had to be resolved first. First, \texttt{clusterProfiler::enrichGO()} raises a hard error rather than returning an empty result when a branch has near-total mapping failure -- precisely the behavior the versioned-Ensembl positive control (Section~\ref{sec:matrix}) is designed to produce -- which \texttt{.run\_one\_enrichment()} now recognizes and recovers from. Second, \texttt{clusterProfiler::enrichWP()} requires an explicit \texttt{organism} argument that neither \texttt{enrichKEGG()} nor \texttt{enrichPathway()} require, which the enrichment wrapper omitted; it now defaults to \texttt{"Homo sapiens"}. Third, only \texttt{enrichGO()} accepts an arbitrary key type; \texttt{enrichKEGG()}, \texttt{enrichPathway()}, and \texttt{enrichWP()} accept only Entrez-family identifiers, so KEGG, Reactome, and WikiPathways are computable only across the two Entrez-space branches (\texttt{entrez\_orgdb\_\_first}, \texttt{entrez\_orgdb\_\_list}) of the six-branch design, at which $F_p\in\{0,0.5,1\}$ -- too coarse for the entropy index or the $\tau_{lo},\tau_{hi}$ classification of Section~\ref{sec:fragility} to carry its intended resolution. GO Biological Process, computable across all six branches, is accordingly the primary namespace comparison this design supports; KEGG/Reactome/WikiPathways contribute an Entrez-policy (first-hit versus keep-all) sub-comparison rather than a full namespace comparison. Running the corrected pipeline end to end against GSE52778 and two additional datasets (GSE159094, a PBMC dexamethasone series; GSE80651, a podocyte series), to test whether pathway fragility is a property of a pathway's gene-set definition or of a single dataset's noise, and anchoring the case study against ChIP-seq-derived GR/NR3C1 target gene sets, is the immediate next step; a fragility table from that pipeline may already be available in the linked repository under \texttt{analysis/airway-prototype/results/} by the time this manuscript is read.
...
Claude (Anthropic; specifically Claude Sonnet 5, accessed via Claude Code) was used throughout this project's preparation: to write the \texttt{idmapaudit} R package and its test suite, to run the analyses in Sections~\ref{sec:real} and~\ref{sec:validation} and re-run them (rather than assert them) whenever a review raised a question about a reported number, and to draft and revise this manuscript's text. Every number in Sections~\ref{sec:real} and~\ref{sec:validation}, including the ones corrected in this version, was produced by executing the accompanying scripts against the underlying data and test suite, not derived by hand or asserted without running code; that code and its outputs are in the linked repository for independent re-execution. What was not done independently by a human is the code's design, the choice of what to compute, and the manuscript's prose, all of which the corresponding author directed and reviewed but did not write line-by-line. Every commit in the linked repository is consequently attributed to "Claude" in its Git authorship metadata (visible in the repository's commit history) rather than to E.Y.Y.\ personally; we disclose this explicitly, following ICMJE and COPE guidance that generative-AI tools cannot be listed as authors and that their use in manuscript and code preparation must be stated rather than left for a reader to discover.
