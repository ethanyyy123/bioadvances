...
\textbf{Results:} We present \texttt{idmapaudit}, an R package and \texttt{targets} pipeline that holds a differential expression result fixed and varies only the gene-ID-mapping strategy used to build the enrichment query list and background, across the primary Ensembl/Entrez/Symbol namespace choice and the one-to-many resolution policy (drop, first-hit, or keep-all), reporting a per-pathway fragility score and entropy index that treat ``always significant'' and ``never significant'' as equally stable and mid-range scores as fragile. The stability-metric core is implemented independently of any Bioconductor dependency and is exercised by 34 unit tests (124 expectations) against hand-computed known-answer cases, all currently passing. Using two independently generated public processings of the airway dexamethasone dataset (GSE52778, both hosted on GEO), we report a real empirical case study: background-choice-sensitive gene-universe divergence between the two processings (69\% on raw annotation counts, 3.7\% once both are restricted to detected genes), real ground-truth differential expression (536/19{,}434 genes at FDR$<0.05$), and a marker-panel calibration check that recovers sample treatment identity from expression alone with only 75\% accuracy, reported as a limitation rather than built upon. Full pathway-level enrichment against real pathway databases is implemented and unit-tested but not yet reported here end to end; a synthetic worked example illustrates the fragility-score mechanics it would populate.\\
...
The natural next step -- comparing File 1's and File 2's differential-expression calls to measure real cross-namespace fragility -- requires knowing which of File 2's 16 GSM-labeled columns received dexamethasone. That metadata is not present in the supplementary FPKM matrix as supplied; it is not, as an earlier draft of this section stated, lost from GSE52778's record (see Section~\ref{sec:limitations} for the correction and what it changes). Using GR-target biology as an orthogonal ground truth, in the spirit of the project's glucocorticoid case-study framing, we attempted to recover the labels directly from expression before we had the series metadata in hand: an 8-gene composite $z$-score built from the same canonical marker panel, clustered with $k$-means ($k=2$), was first calibrated against File 1's \emph{known} labels before being trusted on File 2. Result: only 12 of 16 File-1 samples (75\%) are correctly recovered this way (Fig.~\ref{fig:calibration}, left) -- two Albuterol+Dex samples score as low as the Untreated/Albuterol arm, and two Dex/Untreated samples land on the wrong side of the split, reflecting real inter-donor variability in this dataset that a small marker panel cannot fully average out. Applying the identical, identically-calibrated procedure to File 2 (Fig.~\ref{fig:calibration}, right) produces a superficially clean 8-vs-8 split, but the calibration result means that split cannot be trusted for individual samples at better than roughly 3-in-4 odds. We report the recovered File-2 grouping in the repository for transparency but do not use it for any further differential-expression or fragility comparison: doing so would silently conflate genuine namespace/annotation effects with label-recovery error, which is precisely the kind of confound this project exists to prevent, not commit. Section~\ref{sec:limitations} reports what the actual series metadata, once available, changes.
...
This manuscript reports what has actually been built and run: a fully specified and unit-validated stability-metric framework, a wired-up ID-mapping matrix and enrichment-battery interface, a real empirical case study on GSE52778's two independently-processed public forms (Section~\ref{sec:real}), and a synthetic illustration of the fragility-score mechanics (Section~\ref{sec:example}). An earlier draft of this manuscript reported no pathway-\emph{level} findings (GO/KEGG/Reactome/WikiPathways fragility scores) and attributed this to the Bioconductor annotation packages the full pipeline depends on being network-unreachable in the sandbox used to prepare it. That was true of the sandbox but not, we discovered on review, of the project: the \texttt{targets} pipeline's own continuous-integration job installs the full stack (\texttt{DESeq2}, \texttt{org.Hs.eg.db}, \texttt{clusterProfiler}, \texttt{ReactomePA}, \texttt{biomaRt}, \texttt{fgsea}, \texttt{airway}) without error on a standard GitHub Actions runner, and had done so before this correction was made. The pipeline's actual failure, once it could install and run, was a bug: \texttt{clusterProfiler::enrichGO()} raises a hard error rather than an informative empty result when a branch has near-total mapping failure -- precisely the behavior the versioned-Ensembl positive control (Section~\ref{sec:matrix}) is designed to produce -- so the pipeline was crashing on its own deliberate stress test. This is now fixed (\texttt{.run\_one\_enrichment()} recognizes and recovers from that specific condition; Section~\ref{sec:validation}). We report this plainly rather than let the corrected, narrower version of the blocker go unstated: a paper about auditing invisible pipeline failures should not itself present a bug as a firewall.

Fixing that bug is necessary but not sufficient for a pathway-level result on real data, because of a second, structural limitation the same code-level review surfaced: only \texttt{enrichGO()} accepts an arbitrary key type, so KEGG, Reactome, and WikiPathways can only be run for the two Entrez-space branches (\texttt{entrez\_orgdb\_\_first}, \texttt{entrez\_orgdb\_\_list}) of the six-branch design (Section~\ref{sec:design}). At only two branches, $F_p\in\{0,0.5,1\}$ for those three databases -- too few values for the entropy index or the $\tau_{lo},\tau_{hi}$ classification of Section~\ref{sec:fragility} to carry the resolution the design intends. GO Biological Process, run across all six branches, is therefore the primary analysis a real run of this pipeline supports; KEGG/Reactome/WikiPathways contribute only an Entrez-policy (first-hit vs.\ keep-all) sub-comparison, not a full namespace comparison. Running the now-fixed pipeline end to end against GSE52778 and the two additional planned datasets (GSE159094, a PBMC dexamethasone series, and GSE80651, a podocyte series), to answer whether pathway fragility is a property of the pathway's gene-set definition or of a single dataset's noise, and anchoring the glucocorticoid case study against ChIP-seq-derived GR/NR3C1 target gene sets, remains the immediate next step; by the time this manuscript is read, a fragility table produced by that corrected pipeline may already be in the linked repository under \texttt{analysis/airway-prototype/results/}.

The real case study in Section~\ref{sec:real} was made possible only because the corresponding author supplied the two GSE52778 files directly; it illustrates, on genuine data, both halves of this project's concern -- a background-choice-sensitive gene-universe difference and real symbol-naming anomalies at the namespace/annotation level (Section~\ref{sec:real-universe}), and, in the calibration finding of Section~\ref{sec:real-calibration}, a related but distinct failure mode. We stated that finding narrowly: \emph{the supplementary FPKM matrix we analyzed does not carry per-sample treatment labels}, not that GSE52778's metadata is lost from the record -- a distinction an earlier draft of Section~\ref{sec:real-calibration} blurred, stating the broader claim outright rather than the narrow one; that sentence is corrected in this version to match. The series metadata does in fact carry per-sample labels, as is typical: GSE52778's GEO sample titles and characteristics identify each GSM by donor and treatment (e.g.\ GSM1275862 = donor N61311, untreated; GSM1275863 = N61311, dexamethasone), which we did not have independent network access to confirm but were able to cross-check against File 1's already-known labels via the same GR-target composite score used in Section~\ref{sec:real-calibration}: every sample identity available to us this way is consistent with that composite score's direction (untreated/albuterol samples score low, dexamethasone/albuterol+dexamethasone samples score high), which is what a correct labeling should produce and a wrong one would not reliably. We report this consistency check rather than the full donor-by-treatment reassignment it would take to redo Section~\ref{sec:real-de} as a paired analysis or Section~\ref{sec:real-calibration} as a real cross-namespace comparison, because we have confirmed labels for only a subset of File 2's 16 samples, not all sixteen, and extending a partial, consistent pattern to the rest by assumption is exactly the kind of confident-but-unverified labeling Section~\ref{sec:real-calibration} already declined to build on once. A paired re-analysis of File 1, and the real File-1-vs-File-2 cross-namespace differential-expression comparison this project has been aiming at throughout, are immediate next steps that need the complete metadata, not new code or further network access.
...
Gene-identifier-mapping choice is a routinely invisible step between differential expression and pathway enrichment. \texttt{idmapaudit} turns that choice into a measured, reusable quantity -- the per-pathway fragility score -- built on a stability-metric core that is fully specified, unit-tested against hand-computed answers, and released independently of the heavier Bioconductor dependencies its empirical use requires. Applied to real, public GSE52778 data, the same auditing mindset surfaced genuine findings beyond what was originally planned: a background-choice-sensitive gene-universe difference between two public processings of the identical experiment, a corrected symbol-anomaly rate, and a marker-based sample-relabeling method that only reaches 75\% accuracy even under calibration -- a workaround-fragility problem adjacent to, and compounding, the identifier-mapping problem this tool was built to audit. The pipeline that produces full pathway-level fragility scores installs and, as of this version, runs on real data (Section~\ref{sec:limitations}); this manuscript reports its design and every result actually obtained honestly, including a negative calibration result and a bug found in its own audit tool, rather than results that were not obtained or a blocker that was not the real one.
