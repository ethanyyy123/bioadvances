...
The scale of the problem is not in dispute. \citet{wadi2016outdated} showed that stale Gene Ontology annotations measurably change enrichment calls, and the Bioconductor support forums carry a large, informal record of users losing 20--45\% of a gene list on Ensembl-to-Entrez conversion with no systematic accounting of the downstream pathway-level consequences. What is missing is not awareness that the problem exists, but a reusable instrument for measuring it: something that fixes everything else in a DE-to-enrichment pipeline, varies only the identifier-mapping strategy, and reports which pathway calls are artifacts of that choice rather than of the underlying biology.

The closest published work is \citet{xie2023dexbenchmark}'s Dex-Benchmark, which uses dexamethasone perturbation data and ChIP-seq-derived glucocorticoid-receptor (GR/NR3C1) target gene sets as a silver-standard concordance check -- but to benchmark differential-expression \emph{algorithm} choice (DESeq2 versus limma versus other methods), with the identifier space fixed throughout. \texttt{idmapaudit} inverts that design: it fixes the DE algorithm and result, and varies only the identifier-mapping step, producing a per-pathway \emph{fragility score} that is, to our knowledge, the first attempt to turn "ID mapping is lossy" and "enrichment is sensitive to its inputs" -- two facts usually stated separately -- into a single, quantified, dataset-independent audit.

This paper describes the design and the currently implemented and validated portion of that audit tool. Consistent with treating validity claims carefully, we report only what has actually been run: the core stability-metric logic, unit-tested against hand-derived answers, and a small synthetic worked example that demonstrates the mechanics of the fragility score. The empirical case study against the airway glucocorticoid-response dataset (GSE52778; \citealp{himes2014rnaseq}) is fully specified and implemented as a \texttt{targets} pipeline (Section~\ref{sec:limitations}) but requires a Bioconductor software stack that was not available in the compute environment used to prepare this manuscript, and so is reported here as planned and implemented, not as executed.
...
\midrule
stability-metrics   & 52  & 52  & 0 \\
enrichment-battery  & 29  & 29  & 0 \\
mapping-matrix      & 20  & 20  & 0 \\
fragility-lookup    & 10  & 10  & 0 \\
\midrule
\textbf{Total}      & \textbf{111} & \textbf{111} & \textbf{0} \\
\botrule
\end{tabular*}
\end{table}
...
File 1 annotates 23{,}273 gene identifiers; File 2 annotates 39{,}376 -- a 69\% larger gene universe a decade later (Fig.~\ref{fig:universe}), entirely independent of any per-sample analysis. Since an ORA background list is exactly this gene universe (Section~\ref{sec:matrix}), this single comparison is itself a real, quantified instance of the paper's concern: the same GEO series would be tested against measurably different background sizes depending only on which of its two public processings is used.

\begin{figure}[!t]
\centering
\includegraphics[width=0.75\columnwidth]{fig_universe_size.pdf}
\caption{Number of annotated gene identifiers in each of GSE52778's two public processings: the original 2013 Cufflinks/symbol matrix (File 1) versus a GRCh38.p13/NCBI RefSeq re-processing indexed by Entrez GeneID (File 2).}
\label{fig:universe}
\figalttext[Bar chart]{Two bars: File 1 at 23,273 genes, File 2 at 39,376 genes.}
\end{figure}

A second, string-level, reference-free measurement on File 1 alone: 2{,}633 of its 23{,}273 gene symbols (11.3\%) match at least one non-standard or placeholder naming pattern -- \texttt{LOC}/\texttt{LINC}-prefixed provisional identifiers (6.1\%), \texttt{orf}-style open-reading-frame placeholders (3.8\%), and symbols ending in a bare \texttt{-}\emph{n} suffix (1.4\%) -- none of which is guaranteed to survive unchanged into a current annotation. This is a real, source-level instance of exactly the identifier instability Section~\ref{sec:matrix}'s versioned-Ensembl resolver is designed to catch, requiring no external reference database to measure.

\subsection{Ground-truth differential expression (File 1)}\label{sec:real-de}

Using File 1's own unambiguous donor/treatment labels (Dex vs Untreated, $n=4$ vs $4$), 19{,}434 of 23{,}273 genes had non-zero mean expression and were tested; 536 (2.8\%) were significant at FDR$<0.05$ (Fig.~\ref{fig:volcano}). Of the eight canonical, massively glucocorticoid-receptor-induced marker genes used throughout this case study (FKBP5, TSC22D3/GILZ, ZBTB16, KLF15, PER1, DDIT4, CRISPLD2, PDK4), all eight show the expected positive log-fold change, and three -- FKBP5 ($\log_2$FC$=3.70$, FDR$=0.019$), ZBTB16 ($3.92$, $0.037$), and PER1 ($2.60$, $0.003$) -- clear FDR$<0.05$ under this simple test (Table~\ref{tab:markerde}); the other five have large effect sizes but do not clear FDR$<0.05$ with only four donors and no cross-gene variance shrinkage. This is a genuine, secondary finding in its own right: statistical-method choice (a per-gene $t$-test on FPKM versus DESeq2's negative-binomial shrinkage on counts) also gates which canonical, well-replicated biology clears a significance threshold, compounding whatever instability the identifier-mapping choice separately introduces.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig_real_volcano.pdf}
\caption{Real differential expression, File 1 (Dex vs Untreated, $n=4$ vs $4$, GSE52778): $\log_2$ fold change against $-\log_{10}$(FDR) for all 19{,}434 tested genes. The eight canonical GR-target marker genes are circled.}
\label{fig:volcano}
...
The real case study in Section~\ref{sec:real} was made possible only because the corresponding author supplied the two GSE52778 files directly; it illustrates, on genuine data, both halves of this project's concern -- a 69\% gene-universe difference and real symbol-identifier anomalies at the namespace/annotation level (Section~\ref{sec:real-universe}), and, in the calibration finding of Section~\ref{sec:real-calibration}, a related but distinct failure mode: a reprocessed public dataset can lose the sample-level metadata needed to even attempt a same-samples comparison, and a literature-validated marker panel is not automatically enough to reconstruct it reliably. We deliberately stopped at reporting that 75\% ceiling rather than using the recovered labels for a headline fragility number, because doing so would have reported label-recovery noise as if it were mapping-driven instability -- the same conflation Section~\ref{sec:design} designs against for the DE step, applied here to a metadata-recovery step we did not originally plan for.
...
E.Y.Y.\ designed the audit framework, implemented the software and tests, supplied the GSE52778 data files analyzed in Section~\ref{sec:real}, and wrote the manuscript.
